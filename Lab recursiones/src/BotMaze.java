...
public class BotMaze {
	...
	public static class PartialSolution {
		...
		public int examine() {
			// Add your code here.
			if(this.maze.getPosition(posX, posY)== WALL) {
				return ABANDON;
			}
			else if(this.maze.getPosition(posX, posY)== PATH) {
				return ABANDON;
			}
			if(this.maze.getPosition(posX, posY)== EXIT) {
				return ACCEPT;
			}
			return CONTINUE; // Dummy Return
		}
		...
		public PartialSolution[] extend() {
			// Add your code here.
			ArrayList<PartialSolution> arr = new ArrayList<PartialSolution>();
			PartialSolution p1 = new PartialSolution(posX, posY-1, maze);
			arr.add(p1);
			PartialSolution p2 = new PartialSolution(posX+1, posY, maze);
			arr.add(p2);
			PartialSolution p3 = new PartialSolution(posX, posY+1, maze);
			arr.add(p3);
			PartialSolution p4 = new PartialSolution(posX-1, posY, maze);
			arr.add(p4);
			return arr.toArray(new PartialSolution[arr.size()]); // Dummy Return
		}
